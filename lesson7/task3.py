...
# (__truediv__()).Данные методы должны применяться только к клеткам и выполнять увеличение, уменьшение, умножение и о
# бычное (не целочисленное) деление клеток, соответственно. В методе деления должно осуществляться округление значения
# до целого числа.
...
class Cell:
    ...
    def __sub__(self, other):
        '''уменьшение клеток'''
        return Cell(self.cell_count - other.cell_count) if (self.cell_count - other.cell_count) > 0 else 'Нельзя вычесть'
    ...
    def __truediv__(self, other):
        '''деление клеток с округлением до целого числа'''
        return Cell(round(self.cell_count // other.cell_count))

    def make_order(self, n):
        row = '*' * n + '\n'
        res = row * (self.cell_count // n) + '*' * (self.cell_count % n)
        return f'{res}'
